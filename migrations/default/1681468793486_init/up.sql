...
CREATE TABLE public.invoices (
    ...
    CONSTRAINT total CHECK (((total >= (0)::numeric) AND (total <= (10000)::numeric)))
);
...
CREATE TABLE public.reservations (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT length_minutes CHECK (((length_minutes = 60) OR (length_minutes = 120)))
);
COMMENT ON TABLE public.reservations IS 'Company reservations';
